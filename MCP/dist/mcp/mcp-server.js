...
const HdevParser_1 = require("./HdevParser");
// ==================== 工具实现 ====================
const TOOLS = [
    {
        name: "list_files",
...
class HdevMcpServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    case "list_files":
                        return await this.handleListFiles(typedArgs);
                    case "parse_file":
                        return await this.handleParseFile(typedArgs);
                    case "list_procedures":
                        return await this.handleListProcedures(typedArgs);
                    case "get_procedure":
                        return await this.handleGetProcedure(typedArgs);
                    case "search_code":
                        return await this.handleSearchCode(typedArgs);
                    case "get_parameter_info":
                        return await this.handleGetParameterInfo(typedArgs);
                    case "get_call_graph":
                        return await this.handleGetCallGraph(typedArgs);
                    case "get_file_content":
                        return await this.handleGetFileContent(typedArgs);
                    default:
    ...
    findHdevFiles(directory, pattern) {
        const files = [];
        try {
            const entries = fs.readdirSync(directory, { withFileTypes: true });
            for (const entry of entries) {
    ...
    async handleParseFile(args) {
        const { filePath } = args;
        if (!fs.existsSync(filePath)) {
            throw new Error(`File not found: ${filePath}`);
        }
        const content = fs.readFileSync(filePath, "utf-8");
        const fileInfo = HdevParser_1.HdevParser.parseFile(filePath, content);
        // 更新索引
    ...
    async handleListProcedures(args) {
        let procedures = [];
        if (args.filePath) {
            ...
            if (fileInfo) {
                ...
                if (fs.existsSync(args.filePath)) {
                    ...
                    const fileInfo = HdevParser_1.HdevParser.parseFile(args.filePath, content);
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify(procedures.map(p => ({
                        name: p.name,
                        signature: p.signature,
                        file: p.name,
                    })), null, 2),
    ...
    async handleGetProcedure(args) {
        ...
        if (format === "json") {
            ...
            output = HdevParser_1.HdevParser.formatProcedureForAI(procedure);
        }
        else {
            // plain format
    ...
    async handleSearchCode(args) {
        ...
        if (filePath) {
            ...
            if (fileInfo) {
                indexToSearch = {
                    ...
                    procedureMap: new Map(Array.from(this.index.procedureMap.entries()).filter(([, { fileInfo: f }]) => f.filePath === filePath)),
                    callGraph: new Map(Array.from(this.index.callGraph.entries()).filter(([name]) => {
        ...
        const results = HdevParser_1.HdevParser.searchProcedure(indexToSearch, query);
    ...
    async handleGetParameterInfo(args) {
    ...
    async handleGetCallGraph(args) {
        const { procedureName, depth = 1 } = args;
        let callGraph;
        if (procedureName) {
            // 获取指定 procedure 的调用链
            const chain = HdevParser_1.HdevParser.getCallChain(this.index, procedureName, new Set());
            callGraph = {
